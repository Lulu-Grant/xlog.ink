...
if (mail_recently_sent('edit-link', $limitKey, 600)) {
    api_error('too_frequent', '同一页面的修改邮件 10 分钟内只能重发一次。', 429);
}
$token = bin2hex(random_bytes(32));
$hash = hash('sha256', $token);
db_exec('UPDATE pages SET token_hash = ?, updated_at = ? WHERE slug = ?', [$hash, now_iso(), $slug]);
send_mail_template($email, 'edit-link', ['url' => 'https://' . $slug . '.xlog.ink/', 'edit_url' => rtrim(xlog_config('base_url'), '/') . '/edit.php?t=' . $token]);
